...
# **การบ้านครั้งที่ 6**
<br>[State Machine : R-type](https://www.youtube.com/watch?v=KRL4135NnyI)  

**State Machine** คือ วงจรเชิงลำดับซึ่งออกแบบเป็นสถานะการทำงาน (state) ของวงจรออกเป็นหลายๆ สถานะ แต่ละสถานะมีลอจิกการทำงานที่ต่างกัน เพื่อกำเนิดค่าเอาต์พุตและค่าสถานะถัดไป <br>[อ้างอิง](https://th.wikipedia.org/wiki/%E0%B9%80%E0%B8%84%E0%B8%A3%E0%B8%B7%E0%B9%88%E0%B8%AD%E0%B8%87%E0%B8%AA%E0%B8%96%E0%B8%B2%E0%B8%99%E0%B8%B0%E0%B8%88%E0%B8%B3%E0%B8%81%E0%B8%B1%E0%B8%94)  
ประกอบด้วย  
![image](22 s.jpg) 

**State Machine ของ R-type**  
มีทั้งหมด 4 States  
Cycle 1 *Fetch*  
![image](15 s.jpg)
(สีแดง)  
*MemRead=1* คือ อ่านคำสั่งจาก memory  
*IorD=1* คือ (เส้นสีม่วง) เอาค่าที่ address ชี้จาก PC มาไว้ที่ memory  
*IRWrite=1* คือ (เส้นสีม่วง) เขียนคำสั่งที่ Instruction register  
*ALUsrcA=0* คือ (เส้นสีเขียว) เอา PC เข้าไปใน ALU  
*ALUsrcB=1* คือ (เส้นสีเขียว) Mux ชี้ที่1 เลือก 4 เข้ามใน ALU   
*ALUOP=ADD* คือ ALUOP ควบคุมคำสั่งให้เป็น ADD PC เข้ากับ 4  
*PCWrite=1* คือ (เส้นสีแดง) เอาค่าที่บวกได้เขียนลงบน PC  

Cycle 2 *Decode*  
![image](16 s.jpg)  
*ALUsrcA=0* คือ (เส้นสีแดง) เอาค่าที่PC เข้าไปใน ALU  
*ALUsrcB=3* คือ (เส้นสีแดง) offset ในตอนแรกมี16-bit เปลี่ยนไปเป็น32-bit ที่Sign Extend และ ทำการ Shift left 2 จากนั้นนำไปเก็บไว้ที่ ALU   
*ALUOP=0* คือ (สีม่วง) ควบคุมคำสั่งให้เป็น ADD  
(สีม่วง) เอา rs และ rt จะถูกส่งไป Read Register จากนั้นไป Read Data และถูกพักไว้ที่ A และ B  

Cycle 3 *Execution*  
![image](17 s.jpg)  
*ALUsrcA=1* คือ (สีเขียว) Mux เลือก 1 คือเอาค่าrs ที่ A เข้าไปใน ALU  
*ALUsrcB=0* คือ (สีเขียว) Mux เลือก 0 คือเอาค่าrt ที่ B เข้าไปใน ALU  
*ALUOP=2* คือ (สีม่วง) ALUOP ควบคุมคำสั่งให้เป็น คำสั่ง ตามคำสั่งใน Instruction register  
(สีแดง) เอาค่าที่คำนวณได้ในALUเก็บไว้ที่ALUout  

Cycle 4 *Write Register*  
![image](18s.jpg)   
*RegWrite=1* คือ มีการเขียนบน register  
*MemtoReg=0* คือ (เส้นสีเขียว) เอาค่าที่ALUout ไปใส่ในrd โดย Mux เลือก 0  
*RegDst=1* คือ (เส้นสีม่วง) Mux เลือก 1 เอาrd เข้าไปเก็บใน Write Register  


# **การบ้านครั้งที่ 7**
<br>[Pipelining](https://www.youtube.com/watch?v=oM0MImrtbX0)  

**Pipelining**  
*Pipelining* ในคอมพิวเตอร์ หมายถึงการเคลื่อนย้ายที่ซ้อนกันของคำสั่ง (Over lapped)  
* ทำงาน 1 คำสั่งจบในหลาย cycle  
* แม้คำสั่งแรกยังไม่เสร็จแต่สามารถนำคำสั่งถัดไปมาทำต่อได้เลย  

**ความแตกต่างระหว่าง Single cycle, Multi cycle และ Pipelining**  
![image](19 p.jpg)  
*Single cycle* = 1 คำสั่งจบภายใน 1 Cycle แต่ละCycle จะใช้เวลาเท่ากันทั้งหมด และจะใช้เวลาของคำสั่งที่นานที่สุด  
สมมติว่าคำสั่ง lw ใช้เวลา 3 วินาที และ sw ใช้เวลา 2 วินาที Single cycle จะใช้เวลาทั้งหมด 6 วินาที เพราะ sw จะใช้เวลา 3 วินาทีเหมือนกับ lw   

*Multi cycl* = 1 คำสั่งจบภายในหลาย Cycle แต่จะใช้เวลาตามจริง  
สมมติว่าคำสั่ง lw ใช้เวลา 3 วินาที และ sw ใช้เวลา 2 วินาที Multiple cycle จะใช้เวลาทั้งหมด 5 วินาที   

*Pipelining* = 1 คำสั่งจบภายในหลาย cycle ระยะเวลาแต่ละ cycle คือระยะเวลาของแต่ละคำสั่ง และเมื่อคำสั่งแรกยังไม่เสร็จ ก็สามารถทำคำสั่งที่สองซ้อนไปด้วยได้  
ตัวอย่างที่เข้าใจง่ายๆ คือการซักผ้า    
สมมติว่าการซักผ้า มี 4 ขั้นตอน    
	* 1) ซักผ้า ใช้เวลา 30 นาที  
	* 2) อบผ้า ใช้เวลา 30 นาที   
	* 3) รีดผ้า ใช้เวลา 30 นาที  
	* 4) เอาผ้าไปเก็บที่ตู้ ใช้เวลา 30 นาที  
1 เซ็ตการทำงาน คือ4 ขั้นตอน รวมเป็น 2 ชั่วโมง  
![image](20 p.jpg)  
ตัวอย่างของการทำงานแบบSingle cycle, Multi cycle กับ Pipelining  
ภาพซ้าย คือการทำงานแบบปกติที่จะทำงานทีละเซ็ท ทำทั้งหมด 4 เซ็ท จะใช้เวลา 4 x 2 = 8 ชั่วโมง  
ภาพขวา คือการทำงานแบบpipelining ที่สามารถทำงานซ้อนกันได้ คือ เมื่อซักผ้าของเซ็ทที่1 เสร็จ แล้วนำไปอบแห้ง เครื่องซักผ้าจะว่าง สามารถนำผ้าของเซ็ทที่2 มาซักได้เลยพร้อมๆกับการอบแห้งของเซ็ทที่1 ดังนั้น ในบรรทัด B จะมีเครื่องซักผ้าและเครื่องอบผ้าที่ทำงานพร้อมกัน เมื่อทำงานซ้อนกันไปเรื่อยๆแบบนี้ จะช่วยลดเวลาทำงานลงได้ (30x4)+30+30+30 = 3.5 ชั่วโมง  

*ตัวอย่างการทำงานของชุดคำสั่ง แบบ Pipelining*  
![image](21 p.jpg)  
จะเห็นได้ว่า จะไม่มีการทำงานซ้ำขั้นตอนกัน
